...
# Stations to examine
teststa <- c("BDL", "GLC", "PTS", "MAL", "SRH", "MRZ", "UNI", "FRP", 
             "SAL", "STI","VIC", "HLL", "NSL")
             
testset_q3 <- allsta_q3 %>% filter(station %in% teststa)
testset_q4 <- allsta_q4 %>% filter(station %in% teststa) %>% subset(select = c(1:9,13))
testset_q5 <- allsta_q5 %>% filter(station %in% teststa) %>% subset(select = c(1:9,15))
testall <- cbind(testset_q3, testset_q4, testset_q5)
