...
delta_q2_a <- repeating_values(df = delta_q1, x = 12) %>% 
  mutate(same_total = as.numeric(rowSums(select(., contains("same"))))) %>%
  mutate(Flag_Q2 = ifelse(same_total == max(same_total, na.rm = TRUE),"Y", "N"))
...
# Tally up "same" columns (same_total)
# Create column "delete" - if same_total = x (represented by max(same_total), delete = Y)
sacR_q2_a <- repeating_values(df = sacR_q1, x = 12) %>% 
  mutate(same_total = as.numeric(rowSums(select(., contains("same"))))) %>%
  mutate(Flag_Q2 = ifelse(same_total == max(same_total, na.rm = TRUE),"Y", "N"))
...
# Tally up "same" columns (same_total)
# Create column "delete" - if same_total = x (represented by max(same_total), delete = Y)
sanJR_q2_a <- repeating_values(df = sanJR_q1, x = 12) %>% 
  mutate(same_total = as.numeric(rowSums(select(., contains("same"))))) %>%
  mutate(Flag_Q2 = ifelse(same_total == max(same_total, na.rm = TRUE),"Y", "N"))
...
sacR_q3 <- sacR_q2 %>%
  anti_join(sacR_q3_b, by = c("station", "date"))
...
ggplot(Tdiffs) +
  geom_point(aes(x = datetime, y =Tdiff), colour = )
```
