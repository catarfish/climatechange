...
delta_std_0 <- readRDS("climatechange/WaterTemp/data/delta_H.rds")
# checking station
...
rm(delta_std, sacR_std, sanJR_std)
rm(delta_H_0, sacR_H_0, sanJR_H_0)
gc()
...
#based on station and date
sacR_q1 <- sacR_H %>%
  left_join(sacR_q1_a, by = c("station", "date"))
...
# Values that are flagged
sanJR_q1_b <- sanJR_q1_a %>%
  filter(Flag_QC1 == "Y")

#Join original dataframe with "delete" dataframe based on values NOT in common. 
#based on station and date
sanJR_q1 <- sanJR_H %>%
  left_join(sanJR_q1_a, by = c("station", "date"))
...
delta_q2_a <- repeating_values(df = delta_q1, x = 18) %>% 
...
# Tally up "same" columns (same_total)
# Create column "delete" - if same_total = x (represented by max(same_total), delete = Y)
sacR_q2_a <- repeating_values(df = sacR_q1, x = 20) %>% 
  mutate(same_total = as.numeric(rowSums(select(., contains("same"))))) %>%
  mutate(Flag_QC2 = ifelse(same_total == max(same_total, na.rm = TRUE),"Y", "N")) %>%
  select(-starts_with("same"))
...
sacR_q2 <- sacR_q1 %>%
  left_join(sacR_q2_a,by = c("station", "date"))
...
# Tally up "same" columns (same_total)
# Create column "delete" - if same_total = x (represented by max(same_total), delete = Y)
sanJR_q2_a <- repeating_values(df = sanJR_q1, x = 20) %>% 
   mutate(same_total = as.numeric(rowSums(select(., contains("same"))))) %>%
  mutate(Flag_QC2 = ifelse(same_total == max(same_total, na.rm = TRUE),"Y", "N")) %>%
  select(-starts_with("same"))
...
# This does not only remove rows in "delete" table, but all data that match those dates.
sanJR_q2 <- sanJR_q1 %>%
  left_join(sanJR_q2_a,by = c("station", "date"))
...
sacR_q3_a <- sacR_q2 %>%
  ...
  mutate(QC3 = abs(Temp- lag(Temp, n = 1)))%>%
  mutate(sdev_th = 3 * runSD(Temp, 25))%>%
  mutate(Flag_QC3 = ifelse((Q3 > sdev_th | Q3 > 5), "Y", "N"))%>%
  ungroup()
...
sanJR_q3_b <- sanJR_q3_a %>%
  filter(Flag_QC3 == "Y")

sanJR_q3 <- sanJR_q2 %>%
  left_join(sanJR_q3_a, by = c("station", "date")) 
...
ggplot(delta_distrib, aes(x = station, y = percent.large)) + 
  geom_point()
 #-------------------------------------------------------------
...
ggplot(sacR_distrib, aes(x = station, y = percent.large)) + 
  geom_point()


 #-------------------------------------------------------------

Tdiffs_sanJR <- sanJR_q4 %>%
  group_by(station, date)%>%
  arrange(datetime)%>%
  mutate(Tdiff = Temp - lag(Temp, 1)) %>%
  mutate(large = ifelse(Tdiff>5, "Y", "N")) 

sanJR_distrib <- Tdiffs_sanJR %>%
  group_by(station) %>%
  summarize(mean.tdiff = mean(Tdiff, na.rm = TRUE),
            max.tdiff = max(Tdiff, na.rm = TRUE),
            n.large = sum(large == "Y", na.rm = TRUE),
...
ggplot(sanJR_distrib, aes(x = station, y = percent.large)) + 
  geom_point()
```
...
## Q4: Merge and write files up to this point
...
for (name in levels(allsta_q3$station)) {
  ...
  fn=paste('station_files/stations_qa/preoutlier/',name, "_q4.csv", sep="")
  #Save the CSV file for each station
  write_csv(tmp,fn)
}
```

## Write q4 to global environment
