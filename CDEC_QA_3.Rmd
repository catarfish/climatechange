...
# Filter out stations we are not using (not contiguous, etc.) 
delta_std <- delta_std_0 %>% 
  filter(!station %in% c("CCS","FLT","CNT", "CPP", "DMC", "DYR","ECD", "HBP", "KA0", "ROR", "DV7", "BOY", "ANC"))

sacR_std <- sacR_std_0 %>%
  filter(station != "GLN")

sanJR_std <- sanJR_std_0 %>%
  filter(!station %in% c("GCT", "MHR", "MUP", "OAD", "OBD", "SBC", "DAR", "TPS"))
...
## QA2) Remove if there are more than 5 repeating values in a row
...
# for columns where there are 5 values repeating in a row (in one day), new variable "delete" = 1
delta_q2_a <- delta_q1 %>%
  group_by(station, date) %>%
  arrange(station, date, hour) %>%
  mutate(same1 = ifelse(Temp == lag(Temp, n= 1), 1, 0),
        ...
        delete = ifelse(same1 + same2 + same3 + same4 ==4, 1, 0)) %>%
  ungroup()
...
## Merge and write files up to this point
...
delta_q3$region <- "delta"
sacR_q3$region <- "sacR"
sanJR_q3$region <- "sanJR"
...
delta_q4_b <- left_join(delta_q3, delta_q4_a, by = c("station", "date"))

# Calculate modified Z-score
# 0.6745 = 75th percentile
delta_q4_c <- delta_q4_b %>%
  group_by(station) %>%
...
# Add calculated daily median, ranges to original table
sacR_q4_b <- left_join(sacR_q3, sacR_q4_a, by = c("station", "date"))
...
# Add calculated daily median, ranges to original table
sanJR_q4_b <- left_join(sanJR_q3, sanJR_q4_a, by = c("station", "date"))

# Calculate modified Z-score
# 0.6745 = 75th percentile
sanJR_q4_c <- sanJR_q4_b %>%
  group_by(station) %>%
  mutate(modZ = abs(0.6745*(Temp - med)/MAD))
...
# Add calculated daily median, ranges to original table
delta_q5_b <- left_join(delta_q3, delta_q5_a, by = c("station", "date"))
...
# Add calculated daily median, ranges to original table
sacR_q5_b <- left_join(sacR_q3, sacR_q5_a, by = c("station", "date"))
...
# Add calculated daily median, ranges to original table
sanJR_q5_b <- left_join(sanJR_q3, sanJR_q5_a, by = c("station", "date"))
...
write_csv(sanJR_q4, "station_files/s
          tations_qa/merged/sanJR_q4.csv")
...
## Write station files to global environment
