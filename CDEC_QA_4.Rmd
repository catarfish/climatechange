...
### QC5) Rate of change and Spike test
...
2. If difference between value and the value before it > threshold, it is flagged.
3. If difference between value and the value before it > 5, it is flagged.
...
temp_flags <- temp_q5 %>%
  ungroup() %>%
  mutate(AllFlags = paste0(Flag_QC1, ",", Flag_QC2, ",", Flag_QC3, ",", Flag_QC4, ",", Flag_QC5))
head(temp_flags)

temp_final <- temp_flags %>%
  filter(grepl("N,N,N,N,N", AllFlags))
...
write_csv(temp_flags, "TempData/QC/Temp_filtered.csv")
...
(Removed_station <- temp_flags %>%
  ...
  summarize(Init = n(),
            ...
            Pct_Removed_QC1 = round(QC1/Init*100,1),
            Pct_Removed_QC2 = round(QC2/Init*100,1),
            Pct_Removed_QC3 = round(QC3/Init*100,1),
            Pct_Removed_QC4 = round(QC4/Init*100,1),
            Pct_Removed_QC5 = round(QC5/Init*100,1),
            Pct_Removed_Total = round(QCTot/Init * 100,1) ))

# Overall dataset
Flags <- temp_flags %>%
  filter(grepl("Y", AnyFlag))
print(paste0(round(nrow(Flags)/nrow(temp_q5)*100,2), "% flagged"))
