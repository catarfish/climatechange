...
Temp_Event_OAD <- cdec_query(station = "OAD",
                              sensor_num = sensor1,
                              dur_code = "E",
...
a <- Temp_E_F_Raw_df %>%filter(location_id == "OAD")
...
```{r}
Temp_all_std <- read_rds("Tempdata/Std/Temp_all_std.rds")
```
...
temp_H_0 <- Temp_all_std %>%
  filter(!is.na(Temp)) %>%
  filter(!is.na(datetime)) # need this for anomalize later
