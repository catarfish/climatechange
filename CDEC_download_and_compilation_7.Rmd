...
---
Edited: 11/12/19: Changed to CDECRetrieve
...
## Download and install library: https://cran.r-project.org/src/contrib/Archive/CDECRetrieve/ 
...
BAC_r <- cdec_query("BAC", "25", "E", "2008")
...
### Celsius - 15 min interval 

Below, download stations already in Celsius (Sensor = 146)
...
## Standardize Data and Write files
...
* Load raw files
* Merge and write **raw files** into station_files/stations_raw/merged folder
  + Celsius
  + Delta (+East Bay and Mokelumne)
  + sacR  
  + sanJR (+StanislausR)
* Convert fahrenheit to celsius
* Create a "Temp" column so celsius and fahrenheit have the same column name
* Merge **standardized station** data into larger compiled files and write to station_files/stations_std/merged_std folder

```{r standardizeMerge, message = FALSE}
library(dplyr)
library(lubridate)
library(readr)
...
#Bind all the temperature sensors together into a single dataframe and write raw dataset
...
# make standardized dataset (remove s25 column) and write csv.
...
# SUS and SWE are celsius, will need to be merged in

# Bind stations together and write raw dataset
...
# write csv for Sac River stations
...
# Combine files, write csv for sanJR stations
...
sanJR_r$Temp <- round(sanJR_r$Temp, digits = 1)

# write file for standardized data
...
### If necessary, load the standard merged files in. 
...
## Merge Celsius into Fahrenheit datasets

* Write celsius files to global environment
* Incorporate celsius files into appropriate standardized datasets (delta_std, sacR_std, sanJR_std)

```{r merge celsiusStdF, message = FALSE}
####################
### Write celsius files to global environment
...
# Incorporate celsius files into standardized datasets
...
sanJR_std_f <- rbind(sanJR_std2, SJR_std, VNS_std, SR3_std, DAR_std)
sanJR_std_f$region <- "sanJR"

# Combine files
allsta_std_f <- rbind(delta_std_f, sacR_std_f, sanJR_std_f)
...
write_csv(allsta_std_f, "station_files/stations_std/merged_std/allsta_std_f.csv")

```

## Write "final" individual event files.  

```{r individual std stations, message = FALSE}
library(dplyr)
# if you just want to rewrite one/a few stations: 
#VNS_std <- sanJR_std %>% filter(station =="VNS")
#write_csv(VNS_std, "station_files/stations_std/VNS_std.csv")
# Otherwise, use code below.

#allsta_h_std <- read_csv("station_files/stations_std/merged_std/allsta_h_std.csv")
allsta_std_f$station <- as.factor(allsta_std_f$station)

#Get the list of unique MP names
for (name in levels(allsta_std_f$station)) {
  #Subset the data station
  tmp=subset(allsta_std_f,station==name)
  #Create a new filename for each station. Designate the folder you want the files in.
  fn=paste('station_files/stations_std/',name, "_std_f.csv", sep="")
  #Save the CSV file for each station
  write_csv(tmp, fn)
}
```

## Download raw hourly data. Write files. 
Next, download hourly data - write as raw data
This includes all stations with data.

```{r download raw hourly, message = FALSE}
#### Hourly Stations ####
library(lubridate)
library(dplyr)
library(readr)
...
## Merge and write hourly file. 
1. Merge all hourly fahrenheit files together. 
2. Merge all hourly celsius files together. 
3. Standardize each separately.
4. Merge these together into one big hourly file.
5. Separate this file into region and add "region."
6. Merge again.
7. Write file.

```{r merge and write hourly, message = FALSE}
library(readr)
...
hour_F$Temp <- round(hour_F$Temp, digits = 1)
...
# Combine F and C files into one data frame.
hour_std <- rbind(hour_C, hour_F_convert)

# Load station files to global environment. 
hour_std$station <- as.factor(hour_std$station) # need to factorize the "stations"
# Write to the global environment
for (name in levels(hour_std$station)) {
  temp=subset(hour_std, station==name)
  fn=assign(paste0(name,"_h_std"),temp, envir = globalenv())
}

# Label regions
delta_h_std <- rbind(ANC_h_std, BKS_h_std, CLC_h_std, CLL_h_std, EMM_h_std, FRP_h_std, HLL_h_std,
                     IBS_h_std, JER_h_std, PCT_h_std, PTS_h_std, RIV_h_std, SAL_h_std, STI_h_std,
                     UNI_h_std, VIC_h_std, VOL_h_std, MRZ_h_std,
                     CCS_h_std, CNT_h_std, CPP_h_std, DMC_h_std, DYR_h_std, HBP_h_std, KA0_h_std,
                     ROR_h_std, DV7_h_std)
delta_h_std$region <- "delta"

sacR_h_std <- rbind(BDL_h_std, BLL_h_std, MAL_h_std, NSL_h_std, RVB_h_std, SRH_h_std, GLN_h_std)
sacR_h_std$region <- "sacR"

sanJR_h_std <- rbind(ANH_h_std, MSD_h_std, MTB_h_std, SJL_h_std, VER_h_std, RPN_h_std ,SBC_h_std)
sanJR_h_std$region <- "sanJR"

# Merge into all stations and write csv.
allsta_h_std <- rbind(delta_h_std, sacR_h_std, sanJR_h_std)
write_csv(allsta_h_std, "station_files/stations_std/merged_std/allsta_h_std.csv")

```

## Write individual standardized hourly files. 
Load and write individual standardized files for each hourly station.

```{r individual hourly, message = FALSE}
library(readr)
library(dplyr)
#allsta_h_std <- read_csv("station_files/stations_std/merged_std/allsta_h_std.csv")
allsta_h_std$station <- as.factor(hour_std$station)

#Get the list of unique MP names
for (name in levels(allsta_h_std$station)) {
  #Subset the data station
  tmp=subset(allsta_h_std,station==name)
  #Create a new filename for each station. Designate the folder you want the files in.
...
# For writing one station:
# MRZ_h_std <- allsta_h_std %>% filter(station == "MRZ")
# write_csv(MRZ_h_std, "station_files/stations_std/hourly/MRZ_h_std.csv")

```

## Combining hourly and event data for particular stations:

* Read event data (std_f) to global environment.
* Read hourly data (_h_std) to global environment.
* Merge hourly and event data for those where both files are needed (individual stations).

```{r global hourlyEventMerge, message = FALSE}
# Write files to global environment to be merged

# event
for (name in levels(allsta_std_f$station)) {
  temp=subset(allsta_std_f, station==name)
  fn=assign(paste0(name,"_std_f"),temp, envir = globalenv())
}

# hourly
for (name in levels(allsta_h_std$station)) {
  temp=subset(allsta_h_std, station==name)
  fn=assign(paste0(name,"_h_std"),temp, envir = globalenv())
}

# Merge
IBS_m_std <- rbind(IBS_std_f, IBS_h_std)
write_csv(IBS_m_std,"station_files/stations_std/IBS_m_std.csv")


MTB_m_std <- rbind(MTB_std_f, MTB_h_std)
write_csv(MTB_m_std,"station_files/stations_std/MTB_m_std.csv")


SJL_m_std <- rbind(SJL_std_f, SJL_h_std)
write_csv(SJL_m_std,"station_files/stations_std/SJL_m_std.csv")


GLN_m_std <- rbind(GLN_std_f, GLN_h_std)
write_csv(GLN_m_std,"station_files/stations_std/GLN_m_std.csv")

```

## Merge and write regional datasets.These contain hourly and event data. 

```{r merge single, message = FALSE}
delta_std_single <- rbind(ANC_h_std, BAC_std_f, BET_std_f, BKS_h_std, BLP_std_f, CCS_h_std, CLC_h_std, CLL_h_std,
                          CNT_h_std, CPP_h_std, CSE_std_f, CYG_std_f, DMC_h_std, DV7_h_std, DYR_h_std,
                          EMM_h_std, FAL_std_f, FLT_std_f, FRK_std_f, FRP_h_std, GOD_std_f, GYS_std_f, 
                          GZB_std_f, GZL_std_f, HBP_h_std, HLL_h_std,HON_std_f, HUN_std_f, 
                          IBS_m_std, JER_h_std, KA0_h_std, MDM_std_f, MHO_std_f, 
                          MSL_std_f, OBI_std_f, ODM_std_f, OH4_std_f, OLD_std_f, OSJ_std_f, PCO_std_f, 
                          PCT_h_std, PPT_std_f, PTS_h_std, RIV_h_std, ROR_h_std, RSL_std_f, RYC_std_f, 
                          SAL_h_std, SNC_std_f, STI_h_std, SUR_std_f, TEA_std_f, UNI_h_std,
                          VCU_std_f, VIC_h_std, VOL_h_std, MRZ_h_std, MOK_std_f, NMR_std_f, SMR_std_f)
write_csv(delta_std_single, "station_files/stations_sin/merged/delta_std_sin.csv")

sacR_std_single <- rbind(BDL_h_std, BLL_h_std, DLC_std_f, DWS_std_f, FPT_std_f, GES_std_f, GLN_m_std, 
                         GSS_std_f, HWB_std_f, LIB_std_f, LIS_std_f, MAL_h_std, MIR_std_f, NSL_h_std, 
                         RVB_h_std, RYI_std_f, SDI_std_f, SOI_std_f, SRH_h_std, SRV_std_f, SSI_std_f, 
                         SSS_std_f, SUS_std_f, SUT_std_f, SWE_std_f, SXS_std_f, TMS_std_f, TSL_std_f)
write_csv(sacR_std_single, "station_files/stations_sin/merged/sacR_std_sin.csv")

sanJR_std_single <- rbind(ANH_h_std, BDT_std_f, BIR_std_f, DAR_std_f, DGL_std_f, DSJ_std_f, FCT_std_f, GCT_std_f, 
                          GLC_std_f, GLE_std_f,HLT_std_f, HOL_std_f, LPS_std_f, MAB_std_f, MHR_std_f, MUP_std_f, 
                          MRU_std_f, MRX_std_f, MSD_h_std, MTB_m_std, OAD_std_f, OBD_std_f, OH1_std_f, 
                          ORI_std_f, ORM_std_f, ORQ_std_f, ORX_std_f, PDC_std_f, PRI_std_f, 
                          RRI_std_f, SBC_h_std, SGA_std_f, SJC_std_f, SJD_std_f, SJG_std_f, SJJ_std_f, SJL_m_std, 
                          SJR_std_f, SR3_std_f, TPI_std_f, TRN_std_f, TWA_std_f, TWI_std_f, 
                          VER_h_std, VNS_std_f, WCI_std_f,RIP_std_f, RPN_h_std)
...
### Write individual files (each station now has ONE file containing hourly and event data). 

```{r individual single files, message = FALSE}
library(readr)
